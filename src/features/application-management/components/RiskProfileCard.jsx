...
export const RiskProfileCard = ({ profile }) => {
  const {
    title,
    description,
    noOfFunds,
  ...
  return (
    ...
    >
      <div className="mt-4 grid grid-cols-3 gap-4">
        ...
        <div className="flex flex-col gap-1">
          <span className="text-[11px] text-paragraph/60 text-center">Risk Level</span>
          <span className="text-sm font-semibold text-heading text-center">{riskLevel}</span>
        </div>
        <div className="flex flex-col gap-1">
          <span className="text-[11px] text-paragraph/60 text-right">No. of Risk Slabs</span>
          <span className="text-sm font-semibold text-heading text-right">{noOfRiskSlabs}</span>
        </div>
      </div>
    </Card>
  );
